...
app.get("/my_pic",function(req,res){
  ...
  fs.readdir("albums/"+req.session.username+"/"+dic,function(err,data){
    ...
    for(var i =0;i<data.length;i++){
      ...
      console.log("411"+data[i]);
      console.log("412"+data_img[i]);
    }
    res.render("pics",{
      username:req.session.username,
      hasLogin:req.session.hasLogin,
...
app.post("/uploads",function(req,res){
  ...
  form.parse(req,function(err,fields,files){
    ...
    // console.log(files);
    // console.log(formid.uploadDir+"/"+req.session.username+"/"+album_name+"/"+name);
    fs.rename(files.fileName.path,form.uploadDir+"/"+req.session.username+"/"+album_name+"/"+name,function(err){
      if(err){
        console.log(err);
        res.send("改名失败")
        return;
